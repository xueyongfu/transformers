...
import random
import sys
sys.path.append('/root/deployment/entity_type/')
...
def evaluate(args, model, tokenizer,lines, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, eval_task, tokenizer,lines, evaluate=True)
...
def load_and_cache_examples(args, task, tokenizer,lines, evaluate=False):
    ...
    examples = (
        processor.get_predict_examples(lines) if evaluate else processor.get_train_examples(args.data_dir)
    )
...
def main():
    ...
    parser.add_argument(
        "--model_name_or_path",
        default='/root/models/chinese/bert/pytorch/bert-base-chinese',
        type=str,
    ...
    # 已训练好的模型地址
    pretrained_model_path = os.path.join( args.output_dir, 'checkpoint-11400')
    tokenizer = tokenizer_class.from_pretrained(pretrained_model_path, do_lower_case=args.do_lower_case)
...
def inference_entity_type(lines):
    result, preds, preds_prob = evaluate(_args, _model, _tokenizer,lines, prefix='')
    preds = [index2label[pred] for pred in preds]
    return preds


if __name__ == "__main__":
    lines = [['希望居委尽快处理。上述张杨路1000弄3号门口有居民乱晾晒','管理不善']]
    preds = inference_entity_type(lines)
    print(preds)
